...
sub usage
{
   ...
   print STDERR "        -s : (par d‚faut) G‚n‚ration sur site\n";
   print STDERR "        -h : Help\n";
   print STDERR "\n";
}
...
foreach $ligne (@ouv)
{
   ...
   elsif (($prem eq '+') || ($prem eq 'x'))
   {
      ...
      else
      {
         # erreur, arret
         printf STDERR "*** Error line $nblig ***\n";
         printf STDERR " plus de 5 reeditions ?!\n";
         printf STDERR "$lig\n";
         exit;
      }
   }
...
#     printf STDERR "Oh Oh, une note... !\n";
      ...
      ||  (($ref_en_cours eq "NOUV_REF") && (($prem eq '=') || ($prem eq ':'))))
   {
      ...
      $nota=substr ($lig, $type_start-1, 1);
      $type=substr ($type_c, 0, 1);
      $stype=substr ($type_c, 1, 1);

# Liste precedente basee sur le 'x'
#     $complet=((index($type_c, 'x') == -1) ? 'OUI' : 'NON');
      if    ($nota eq "=") { $complet='OUI'; } # OK
      elsif ($nota eq "_") { $complet='OUI'; } # OK, mais moins de certitudes
      elsif ($nota eq "k") { $complet='OUI'; } # A priori complet, … confirmer
      elsif ($nota eq "c") { $complet='OUI'; } # ordre … confirmer
      elsif ($nota eq "p") { $complet='NON'; } # contenu partiel
      elsif ($nota eq "i") { $complet='NON'; } # inconnu ou quasi
      elsif ($nota eq " ") { $complet='OUI'; } # Pas fait
      else                 { $complet='OUI'; printf STDERR "*** Error ligne $nblig [$nota][$type_c] ***\n"; } # erreur
       
#A FAIRE      if ($nota ne ' ') $complet = 'NON';
# 'i' = inconnu, ou quasi - A compl‚ter
# 'p' = partiel - A compl‚ter
# 'c' = complet sauf ‚ventuellement ordre - A revoir
# ' ' = non renseign‚ - A valider
# 'k' = a priori complet (sauf non-fictions)
# '_' = complet (sauf non-fictions)
# '=' = complet - V‚rifi‚ GR avec le volume
   ...
   else
   {
      ...
      # sommaire incomplet
      if (($reference->{COMPLET} eq "NON") &&
          ($stype ne " ") &&
          (($type eq "N") || ($type eq "R") || ($type eq "r") || ($type eq "A") ||
           ($type eq "C") || ($type eq "P") || ($type eq "T") || ($type eq "U")))
...
# Affichage resultats
#---------------------
$outf="${livraison_site}/manque_somm.php";
print STDERR "resultat dans $outf\n";
open (OUTP, ">$outf");
$canal=OUTP;

&web_begin ($canal, "../commun/", "Sommaires manquants ou incomplets");
...
&web_menu (1, "site");
   ...
   &web_data ("<img src='../images/sep.png' alt='--&gt;'/> Manque sommaires\n");
   &web_data ("</div>\n");
&web_data ("<h1>Sommaires incomplets ou inconnus</h1>\n\n");
...
print $canal &tohtml("Nombre d'enregistrements r&eacute;f&eacute;renc&eacute;s auxquels il manque le sommaire : <strong><em>$#tri_refs</em></strong><br />\n");
print $canal &tohtml("M‚mo :<br />[Rec Nvl] = Recueil de nouvelles [Rec-Omn] = Recueil ou Omnibus [Novelis] = Novelisation<br />\n");
print $canal &tohtml("[Antho] = Anthologie [Nvlle] = Nouvelle [Novella] = Court roman<br />\n");
print $canal &tohtml("Les noms des anthologistes sont les bienvenus (… noter qu'ils ne sont pas indiqu‚s ici mˆme s'ils sont connus)<br />\n");
print $canal &tohtml("Outre le sommaire, si vous le pouvez (et/ou si les informations sont mentionn‚es) les donn‚es suivantes nous int‚ressent ‚galement : ISBN, date exacte de parution, anthologiste, genre, titre vo et date de publication originale des textes contenus.<br /><br />");
...
sub AFFICHE
{
   ...
   if ($aa->{AUTEUR}[0] ne $oldaa->{AUTEUR}[0])
   {
      ...
      print $canal &tohtml("<br /><a class=\"auteur\" href=\"../auteurs/$initiale_lien/$lien_auteur.php\">");
...
#   print $canal &tohtml("<span class=\"fr\">$aa->{TITRE}</span> (");
      ...
      $cycle=$aa->{CYCLE}->{CP};
      $scycle=$aa->{CYCLE_S}->{CS};
      $idrec=idrec($titre_seul, $cycle, $scycle);
      $url_antho=url_antho($idrec);
      $url_antho="${url_antho}.php";
      print $canal &tohtml("<span class='fr'><a class='antho' href='../recueils/pages/$url_antho'>$titre</a></span> (");
